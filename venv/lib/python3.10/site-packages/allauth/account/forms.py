...
class LoginForm(forms.Form):
    password = PasswordField(label=_("Senha"), autocomplete="senha atual")
    remember = forms.BooleanField(label=_("Lembrar de mim"), required=False)

    user = None
    error_messages = {
        "account_inactive": _("Esta conta está inativa."),
        "email_password_mismatch": _(
            "O endereço de e-mail e/ou senha que você especificou não estão corretos."
        ),
        "username_password_mismatch": _(
            "O nome de usuário e/ou senha que você especificou não estão corretos."
        ),
    }
    ...
    def __init__(self, *args, **kwargs):
        ...
        if app_settings.AUTHENTICATION_METHOD == AuthenticationMethod.EMAIL:
            ...
            login_widget = forms.TextInput(
                attrs={"placeholder": _("Nome"), "autocomplete": "nome"}
            )
...
class BaseSignupForm(_base_signup_form_class()):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if email_required:
            ...
            self.fields["email"].label = gettext("Email (opcional)")
    ...
    def __init__(self, *args, **kwargs):
        ...
        if app_settings.SIGNUP_PASSWORD_ENTER_TWICE:
            self.fields["password2"] = PasswordField(
                label=_("Senha (novamente)"), autocomplete="new-password"
            )
...
class AddEmailForm(UserForm):
    email = forms.EmailField(
        ...
        required=True,
        widget=forms.TextInput(
            attrs={"type": "email", "placeholder": _("Endereço de email")}
        ),
    )
...
class ChangePasswordForm(PasswordVerificationMixin, UserForm):
    oldpassword = PasswordField(
        label=_("Senha atual"), autocomplete="current-password"
    )
    password1 = SetPasswordField(
        label=_("Nova senha"),
        help_text=password_validation.password_validators_help_text_html(),
    )
    password2 = PasswordField(label=_("Nova senha (novamente)"))
